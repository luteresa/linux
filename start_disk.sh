...
# for $1--xxx.img
#qemu-system-aarch64  -nographic  -machine virt -cpu cortex-a57  -m 1024 -smp 8 -kernel $OUTDIR/arch/arm64/boot/Image   --append "noinitrd root=/dev/vda rw console=ttyAMA0  earlycon=pl011,0x9000000 loglevel=8 memblock=debug "  -drive if=none,file=$1,id=hd0 -device virtio-blk-device,drive=hd0 -device virtio-net-device,netdev=net0 -netdev tap,id=net0,ifname=tap0,script=no,downscript=no

qemu-system-aarch64  -nographic  -machine virt -cpu cortex-a57  -m 1024 -smp 8 -kernel $OUTDIR/arch/arm64/boot/Image   --append "noinitrd root=/dev/vda rw console=ttyAMA0  loglevel=8 memblock=debug "  -drive if=none,file=$1,id=hd0 -device virtio-blk-device,drive=hd0 -device virtio-net-device,netdev=net0 -netdev tap,id=net0,ifname=tap0,script=no,downscript=no
