...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (likely(pte_same(*vmf->pte, vmf->orig_pte))) {
		...
		page_add_new_anon_rmap(new_page, vma, vmf->address);    ///new_page添加到RMAP系统中
...
///处理vma可写情况
	...
	page_add_new_anon_rmap(page, vma, vmf->address);    ///匿名页面添加到rmap系统
...
///pte不为空
	...
	if (vmf->flags & (FAULT_FLAG_WRITE|FAULT_FLAG_UNSHARE)) {   ///FAULT_FLAG_WRITE标志，根据ESR_ELx_WnR设置
		if (!pte_write(entry))
