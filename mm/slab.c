								...
								int nodeid)
{
	...
	folio = (struct folio *) __alloc_pages_node(nodeid, flags, cachep->gfporder);   ///从伙伴系统申请分配页面
	if (!folio) {
...
static void *cache_alloc_refill(struct kmem_cache *cachep, gfp_t flags)
{
	...
	while (batchcount > 0) {
		...
		slab = get_first_slab(n, false);   ///从同一个节点的其他slabs_partial链表或slabs_free链表返回一个可用slab(第一个页面)
		if (!slab)
			goto must_grow;

		check_spinlock_acquired(cachep);

		batchcount = alloc_block(cachep, ac, slab, batchcount);   ///从slab迁移batchcout个对象到本地缓冲池
		fixup_slab_list(cachep, n, slab, &list);
	}
...
direct_grow:     ///整个内存节点没有slab空闲对象
	if (unlikely(!ac->avail)) {
		...
		slab = cache_grow_begin(cachep, gfp_exact_node(flags), node);  ///重新分配slab分配器(包含多个缓冲对象)
		...
		ac = cpu_cache_get(cachep);
		if (!ac->avail && slab)
			alloc_block(cachep, ac, slab, batchcount);   ///从刚分配的slab分配器迁移空闲对象到本地
		cache_grow_end(cachep, slab);                    ///刚分配的分配器添加到合适队列中，这里应该添加到salbs_partial链表中
